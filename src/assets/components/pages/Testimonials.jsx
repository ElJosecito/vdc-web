      ...
      <div className='max-w-screen-xl w-full py-20'>
        <section className='max-w-screen-xl w-full'>
          <div className='flex flex-col items-center'>
            <h2 className='text-4xl font-semibold text-[#011632] text-center md-text-start'>{t('testimonials.title')}</h2>
            ...
            {testimonials.map((item) => (
              <div key={item.id} className='md:w-[413px] md:h-[290px] rounded-lg flex flex-col justify-center items-start relative bg-white p-5 px-7 border-2 border-gray-400 transition-colors duration-150 hover:border-gray-500'>
                <div className='flex gap-4'>
                  ...
                  <div className=''>
                    <p className='text-[#011632] text-base font-semibold mt-4'>{item.author}</p>
                    <div className='flex justify-center items-center gap-2'>
                      {Array.from({ length: item.stars }).map((_, i) => (
                        <BsStarFill key={i} className='text-[#D9B36C]' />
                      ))}
                    </div>
                  </div>
                </div>
                <p className='text-[#011632] text-base pr-8 mt-4'>{item.text}</p>
              </div>
            ))}
          </div>
        </section>
